...
public class CategoryPresenter extends BasePresenter<CategoryContract.Model, CategoryContract.View> {
    ...
    public void search(String stringExtra,int page, Context context) {
        RequestUtils.search(stringExtra, page,new BaseCallBack(context) {
            @Override
            protected void onSuccess(String jsonString) {
                SearchResultBean searchResultBean = GsonUtils.parseJsonWithGson(jsonString, SearchResultBean.class);
                mRootView.searchSucc(searchResultBean);
            }
        });
    }
}
